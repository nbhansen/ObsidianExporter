...
class TestOutlineSchemaCompliance:
    ...
    SUPPORTED_NODE_TYPES = {
        "doc", "paragraph", "heading", "bullet_list", "list_item", 
        "code_block", "blockquote", "horizontal_rule", "image", "text"
    }
    
    SUPPORTED_MARK_TYPES = {
        "strong", "em", "code_inline", "strikethrough", "underline", 
        "highlight", "link", "comment", "placeholder"
    }
    ...
    def test_specific_node_names(self):
        ...
        # Test list items
        list_nodes = [node for node in result.content if node.get("type") == "bullet_list"]
        if list_nodes:
...
[Link](https://example.com)
    ...
    def test_code_block_attributes(self):
        ...
        result = self.generator.convert_markdown("```python\nprint('hello')\n```")
        code_blocks = [node for node in result.content if node.get("type") == "code_block"]
    ...
    def test_heading_attributes(self):
        ...
        for level in range(1, 7):  # H1 through H6
            ...
            result = self.generator.convert_markdown(markdown)
            headings = [node for node in result.content if node.get("type") == "heading"]
    ...
    def test_image_attributes(self):
        ...
        if images:  # Only test if images are generated
            for image in images:
                ...
                if "alt" in attrs:
                    assert isinstance(attrs["alt"], (str, type(None))), "alt should be string or None"
                if "title" in attrs:
                    assert isinstance(attrs["title"], (str, type(None))), "title should be string or None"
    ...
    def test_outline_database_constraints(self):
        ...
        test_content = [
            ...
            TransformedContent(
                ...
                metadata={"title": "This is an extremely long document title that definitely exceeds the one hundred character limit and should be truncated by our generator to meet Outline database requirements which have a strict 100 character maximum"},
                assets=[],
                warnings=[]
            ),
        ]
        ...
        result = outline_gen.generate_outline_package(test_content, very_long_vault_name)
        
        # Test Collection constraints
        for collection in result.collections:
            # urlId must be exactly 10 characters
            url_id = collection.get("urlId", "")
            assert len(url_id) == 10, f"Collection urlId '{url_id}' must be exactly 10 characters, got {len(url_id)}"
            assert url_id.isalnum(), f"Collection urlId '{url_id}' should contain only alphanumeric characters for URL safety"
            
            # name must be 100 characters or less (CollectionValidation.maxNameLength)
            name = collection.get("name", "")
            assert len(name) <= 100, f"Collection name '{name}' must be 100 characters or less, got {len(name)}"
            
            # icon must be 50 characters or less (if present)
            icon = collection.get("icon")
            if icon is not None:
                assert len(icon) <= 50, f"Collection icon '{icon}' must be 50 characters or less, got {len(icon)}"
            ...
            if color is not None:
                ...
                if color:  # If not empty
                    assert color.startswith("#"), f"Collection color '{color}' should be hex color starting with #"
        
        # Test Document constraints
        for doc_id, document in result.documents.items():
            # urlId must be exactly 10 characters
            url_id = document.get("urlId", "")
            assert len(url_id) == 10, f"Document urlId '{url_id}' must be exactly 10 characters, got {len(url_id)}"
            assert url_id.isalnum(), f"Document urlId '{url_id}' should contain only alphanumeric characters for URL safety"
            
            # title must be 100 characters or less (DocumentValidation.maxTitleLength)
            title = document.get("title", "")
            assert len(title) <= 100, f"Document title '{title}' must be 100 characters or less, got {len(title)}"
        ...
        for collection in result.collections:
            ...
            assert len(collection_id) == 36, f"Collection ID should be UUID format (36 chars), got {len(collection_id)}"
            assert collection_id.count("-") == 4, f"Collection ID should be UUID format with 4 hyphens"
